...
public class ProjectServiceVertxEBProxy implements ProjectService {
  ...
  public void getProject(String projectId, Handler<AsyncResult<Project>> resulthandler) {
    ...
    DeliveryOptions _deliveryOptions = (_options != null) ? new DeliveryOptions(_options) : new DeliveryOptions();
    _deliveryOptions.addHeader("action", "getProject");
  ...
  public void getStatusProject(String theStatus, Handler<AsyncResult<List<Project>>> resulthandler) {
    if (closed) {
      resulthandler.handle(Future.failedFuture(new IllegalStateException("Proxy is closed")));
      return;
    }
    JsonObject _json = new JsonObject();
    _json.put("theStatus", theStatus);
    DeliveryOptions _deliveryOptions = (_options != null) ? new DeliveryOptions(_options) : new DeliveryOptions();
    _deliveryOptions.addHeader("action", "getStatusProject");
    _vertx.eventBus().<JsonArray>send(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        resulthandler.handle(Future.failedFuture(res.cause()));
      } else {
